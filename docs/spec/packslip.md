...
7. Check `requires` against the host before installing, and report a
   ...
   cannot check. Between two artifacts that fit, prefer the one whose
...
- In a release job: `uses: jdx/packslip@v1` with `artifacts: dist/*`
  ...
  `resources`, one `--resource` value per line, the rest, and `require`,
  one `--require` value per line, the commands they need. A monorepo runs
...
- `packslip create --project NAME --version X --out dist --url-base URL
  ...
  - Host requirements: `--require bin:NAME[@MIN]`, as in
    ...
    `--no-libs` leaves the artifacts unopened.
